...
//НАЧАЛО
<программа> ::= <импорт модулей><символы разделения строк><функции><символы разделения строк><функция main>

<символы разделения строк> ::= \n| |\t | <символы разделения строк><символы разделения строк>


//Импорт модулей
<импорт модулей> ::= <импорт модуля>|<импорт модуля><символы разделения строк><импорт модулей>
<импорт модуля> ::= import "<адрес и название файла>"
<адрес и название файла> ::= <путь><название файла>
...
<функция> ::= fn <имя функции>(<аргументы функции>):<тип значения>\n<тело функции>\n
<имя функции> ::= <идентификатор>
<идентификатор> ::= <буква> | цифра | _ | <буква><сложный идентификатор>
<сложный идентификатор> ::= <буква><сложный идентификатор> | <цифра><сложный идентификатор> | <сложный идентификатор>
<аргументы функции> ::= <аргумент> | <аргумент>, <аргументы функции>
<аргумент> ::= <имя переменной>
<имя переменной> ::= <идентификатор>
<тип значения> ::= none | | bool | double | array | string
<тело функции> ::= \t<ветвление>|\t<циклы>|\t<операции>| \t<ветвление>\n\t<тело функции>|\t<циклы>\n\t<тело функции>|\t<операции>\n\t<тело функции>

//Условие
<ветвление> ::= if(<условие>):\n<тело функции>|if(<условие>):\n<тело функции>\nelse:<тело функции>if(<условие>):\n<тело функции>\nelif:<тело функции>
<условие> ::= <имя переменной> <оператор сравнения> <имя переменной> | <сложное условие>
<сложное условие> ::= (<условие>) <логический оператор> <сложное условие> | <условие>


//циклы
<циклы> ::= for(<объявление переменной или пустота><условие или пустота><операция или пустота>):<тело функции>
<объявление переменной или пустота> ::= <бинарная операция присвоение>; | ;
<условие или пустота> ::= <условие>; | ;
<операция или пустота> ::= <оперкция>; | ;


//Присвоение
<операции> ::= <операция> | <операция><операции>
<операция> ::= <унарная операция> | <бинарная операция присвоение>
<унарная операция> ::= <имя переменной>++ | <имя переменной>-- | <имя функции>(<аргументы функции>) | break
<бинарная операция присвоение> ::= <имя переменной> = <бинарная операция> | <имя переменной> = <тип данных> | <имя переменной> = <имя функции>(<аргументы функции>) | <тип значения> <имя переменной> = <тип данных>
<бинарная операция> ::= <значение> <оператор> <значение> | <бинарная операция> <оператор> <значение> | <значение> <оператор> <бинарная операция>
<значение> ::= <имя переменной> | <имя функции>(<аргументы функции>) | <тип данных>


//Типы данных
<тип данных> = none | <bool> | <string> | <double> | <array>
<bool> = true | false
...
<цифра> = [0-9]
<array> = <имя переменной>[<число>]

//TO DO

<встроенные функции> ::= char(<операция>) | char(<переменная>) | len(<переменная>) | print(<имя переменной>) | input<имя переменной>
```
